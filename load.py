...
def clean_low_freq_locations(data, poi, min_loc_visits, min_user_checkins):
    raw_checkins = len(data)
    raw_users = len(np.unique(data[:, 0].astype(int)))
    raw_locs = len(np.unique(data[:, 1].astype(int)))

    loc_ids, loc_counts = np.unique(data[:, 1].astype(int), return_counts=True)
    keep_locs = loc_ids[loc_counts >= min_loc_visits]
    keep_loc_mask = np.isin(data[:, 1].astype(int), keep_locs)
    data = data[keep_loc_mask].copy()

    if len(data) == 0:
        raise ValueError('No check-ins left after low-frequency POI filtering.')

    user_ids, user_counts = np.unique(data[:, 0].astype(int), return_counts=True)
    keep_users = user_ids[user_counts >= min_user_checkins]
    keep_user_mask = np.isin(data[:, 0].astype(int), keep_users)
    data = data[keep_user_mask].copy()

    if len(data) == 0:
        raise ValueError('No users left after min_user_checkins filtering.')

    keep_locs = np.unique(data[:, 1].astype(int))
    loc_map = np.zeros(int(keep_locs.max()) + 1, dtype=np.int64)
    loc_map[keep_locs] = np.arange(1, len(keep_locs) + 1)
    data[:, 1] = loc_map[data[:, 1].astype(int)]

    keep_users = np.unique(data[:, 0].astype(int))
    user_map = np.zeros(int(keep_users.max()) + 1, dtype=np.int64)
    user_map[keep_users] = np.arange(1, len(keep_users) + 1)
    data[:, 0] = user_map[data[:, 0].astype(int)]

    poi_lookup = {int(row[0]): row.copy() for row in poi}
    clean_poi_rows = []
    for old_loc in keep_locs:
        row = poi_lookup.get(int(old_loc))
        if row is None:
            row = poi[int(old_loc) - 1].copy()
        clean_poi_rows.append(row)
    clean_poi = np.array(clean_poi_rows)
    clean_poi[:, 0] = np.arange(1, len(clean_poi) + 1)

    stats = {
        'raw_checkins': raw_checkins,
        'raw_users': raw_users,
        'raw_locs': raw_locs,
        'clean_checkins': len(data),
        'clean_users': len(np.unique(data[:, 0].astype(int))),
        'clean_locs': len(np.unique(data[:, 1].astype(int))),
        'dropped_checkins': raw_checkins - len(data),
        'min_loc_visits': min_loc_visits,
        'min_user_checkins': min_user_checkins,
    }
    return data, clean_poi, stats


def process_traj(dname, output_dname=None, min_loc_visits=1, min_user_checkins=0):  # start from 1
    # data (?, [u, l, t]), poi (L, [l, lat, lon])
    output_dname = output_dname or dname
    ...
    if min_loc_visits > 1:
        if min_user_checkins <= 0:
            min_user_checkins = 6
        data, poi, clean_stats = clean_low_freq_locations(data, poi, min_loc_visits, min_user_checkins)
        np.save('./data/' + output_dname + '.npy', data)
        np.save('./data/' + output_dname + '_POI.npy', poi)
        print('clean_stats:', clean_stats)

    # add the code below if you are using dividing time into minutes instead of hours
    data[:, -1] = np.array(data[:, -1] / 60, dtype=int)
    num_user = int(np.max(data[:, 0]))  # max id of users, i.e. NUM
    ...
    for u_id in range(num_user+1):
        ...
        if len(user_traj) == 0:
            continue
        if min_user_checkins > 0 and len(user_traj) < min_user_checkins:
            continue
    ...
    data_pkl = './data/' + output_dname + '_data.pkl'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dname', type=str, default=None)
    parser.add_argument('--min_loc_visits', type=int, default=1)
    parser.add_argument('--min_user_checkins', type=int, default=0)
    args = parser.parse_args()
    process_traj(
        args.dname,
        output_dname=args.output_dname,
        min_loc_visits=args.min_loc_visits,
